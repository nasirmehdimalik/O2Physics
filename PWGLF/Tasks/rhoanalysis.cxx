...
struct rhoanalysis {
  ...
  void init(o2::framework::InitContext&)
  {
    ...
    if(isMixed){ histos.add("hrhomassmixed", "Invariant mass of rho mixed event", kTHnSparseD, {invmassAxis, ptAxiss,  multAxis, rapidityAxis});
  ...
  void processSame(Event::iterator const& events, TrackPi const& tracks)
  {
    ...
    if(cMultiplicityType ==1)multiplicity = events.multTPC();
    else if(cMultiplicityType ==2)multiplicity = events.multFT0M();
    else multiplicity = events.multFV0M();
  ...
  void processMixedEvent(Event const& events, TrackPi const&tracks)
  {
    ...
    for (auto& [c1,tracks1,c2,tracks2] : pair) {
      ...
      float multiplicity = 0;
       if(cMultiplicityType ==1)multiplicity = c1.multTPC();
       else if(cMultiplicityType ==2)multiplicity = c1.multFT0M();
       else multiplicity = c1.multFV0M();
